...
bool g_bBotGlovesLoaded = false;
JSONObject g_jsonGloveConfig = null;

bool g_bBotKnivesLoaded = false;
...
int g_iKnifeSkin[MAXPLAYERS + 1];
float g_fKnifeWear[MAXPLAYERS + 1];
int g_iKnifeSeed[MAXPLAYERS + 1];
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_reloadbotconfigs", Command_ReloadBotConfigs);
}
...
bool GetBotGloveInfo(const char[] szName, int &defIndex, int &skin, float &wear, int &seed)
{
	if (!g_bBotGlovesLoaded || g_jsonGloveConfig == null)
        return false;
    if (g_jsonGloveConfig.HasKey(szName))
    {
        JSONObject botConfig = view_as<JSONObject>(g_jsonGloveConfig.Get(szName));
        if (botConfig != null)
        {
            defIndex = botConfig.GetInt("definition_index");
            skin = botConfig.GetInt("skin");
            wear = botConfig.GetFloat("wear");
            seed = botConfig.GetInt("seed");
    ...
    BuildPath(Path_SM, szPath, sizeof(szPath), "data/bot_gloves.json");
    if (!FileExists(szPath))
    {
        PrintToServer("[BotGloves] File not found: %s", szPath);
        return false;
    }
    
    JSONObject jsonRoot = JSONObject.FromFile(szPath);
    if (jsonRoot == null)
    {
        PrintToServer("[BotGloves] Failed to parse JSON file: %s", szPath);
        return false;
    }
    
    // Check if the bot name exists in our JSON
    if (!jsonRoot.HasKey(szName))
    {
        delete jsonRoot;
        return false;
    }
    
    // Get the bot's glove configuration
    JSONObject botConfig = view_as<JSONObject>(jsonRoot.Get(szName));
    if (botConfig == null)
    {
        delete jsonRoot;
        return false;
...
bool GetBotKnifeInfo(const char[] szName, int &defIndex, int &skin, float &wear, int &seed)
{
    if (!g_bBotKnivesLoaded || g_jsonKnifeConfig == null)
        return false;
        
    if (g_jsonKnifeConfig.HasKey(szName))
    {
        JSONObject botConfig = view_as<JSONObject>(g_jsonKnifeConfig.Get(szName));
        if (botConfig != null)
        {
            defIndex = botConfig.GetInt("definition_index");
            skin = botConfig.GetInt("skin");
    ...
    char szPath[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, szPath, sizeof(szPath), "data/bot_knives.json");
    if (!FileExists(szPath))
    {
        PrintToServer("[BotKnives] File not found: %s", szPath);
        return false;
    }
    ...
    if (jsonRoot == null)
    {
        PrintToServer("[BotKnives] Failed to parse JSON file: %s", szPath);
        return false;
    }
	...
	if (IsValidClient(client))
	{
		if (eItems_AreItemsSynced())
		{
	        ...
	        if (IsFakeClient(client))
			{
			    ...
			    int gloveDefIndex, gloveSkin, gloveSeed;
			    float gloveWear;
			    
			    // Try to get custom glove settings for this bot
			    if (GetBotGloveInfo(botName, gloveDefIndex, gloveSkin, gloveWear, gloveSeed))
			    {
			        g_iStoredGlove[client] = gloveDefIndex;
			        g_iGloveSkin[client] = gloveSkin;
			        g_fGloveWear[client] = gloveWear;
			        g_iGloveSeed[client] = gloveSeed;
			    }
			    else
			    {
			        // Fall back to random glove generation
			    ...
			    int knifeDefIndex, knifeSkin, knifeSeed;
			    float knifeWear;

			    if (GetBotKnifeInfo(botName, knifeDefIndex, knifeSkin, knifeWear, knifeSeed))
				{
				    g_iStoredKnife[client] = knifeDefIndex;
				    g_iSkinDefIndex[client][knifeDefIndex] = knifeSkin;
				    g_fWeaponSkinWear[client][knifeDefIndex] = knifeWear;
				    g_iWeaponSkinSeed[client][knifeDefIndex] = knifeSeed;
				}
				else
				{
				    g_iStoredKnife[client] = g_iKnifeDefIndex[Math_GetRandomInt(0, sizeof(g_iKnifeDefIndex) - 1)];

				    int iKnifeWeaponNum = eItems_GetWeaponNumByDefIndex(g_iStoredKnife[client]);
				    int iRandomKnifeSkin = Math_GetRandomInt(0, g_ArrayWeapons[iKnifeWeaponNum].Length - 1);

				    if (iRandomKnifeSkin != -1)
				        g_iSkinDefIndex[client][g_iStoredKnife[client]] = g_ArrayWeapons[iKnifeWeaponNum].Get(iRandomKnifeSkin);

				    g_fWeaponSkinWear[client][g_iStoredKnife[client]] = Math_GetRandomFloat(0.06, 0.80);
				    g_iWeaponSkinSeed[client][g_iStoredKnife[client]] = Math_GetRandomInt(1, 1000);
				}
			}
...
void SetWeaponProps(int client, int iEntity)
{
    ...
    if (iDefIndex > -1)
    {
        // Declare these variables at the beginning of the function scope
        CEconItemView pItem;
        CAttributeList pDynamicAttributes;
        
        // Check if this is a knife and the client has a custom knife
        if (eItems_IsDefIndexKnife(iDefIndex) && g_iStoredKnife[client] > 0)
        {
            int knifeDefIndex = g_iStoredKnife[client];
            
            // Get the bot's name for direct lookup
            char botName[MAX_NAME_LENGTH];
            GetClientName(client, botName, sizeof(botName));
            
            // Values to apply
            int knifeSkin, knifeSeed;
            float knifeWear;
            
            // Try to get custom knife settings directly from JSON
            if (GetBotKnifeInfo(botName, knifeDefIndex, knifeSkin, knifeWear, knifeSeed))
            {               
                SetEntProp(iEntity, Prop_Send, "m_iItemIDLow", g_iItemIDLow[client][knifeDefIndex]);
                SetEntProp(iEntity, Prop_Send, "m_iItemIDHigh", g_iItemIDHigh[client][knifeDefIndex]);
                ...
                pItem = PTaH_GetEconItemViewFromEconEntity(iEntity);
                pDynamicAttributes = pItem.NetworkedDynamicAttributesForDemos;
                
                // Apply the JSON values directly
                pDynamicAttributes.SetOrAddAttributeValue(6, float(knifeSkin));
                pDynamicAttributes.SetOrAddAttributeValue(7, float(knifeSeed));
                pDynamicAttributes.SetOrAddAttributeValue(8, knifeWear);
            }
            else
            {
                // Fallback to using stored values
                SetEntProp(iEntity, Prop_Send, "m_iItemIDLow", g_iItemIDLow[client][knifeDefIndex]);
                SetEntProp(iEntity, Prop_Send, "m_iItemIDHigh", g_iItemIDHigh[client][knifeDefIndex]);
                SetEntProp(iEntity, Prop_Send, "m_OriginalOwnerXuidLow", GetBotAccountID(client));
                SetEntProp(iEntity, Prop_Send, "m_OriginalOwnerXuidHigh", 17825793);
                ...
                pDynamicAttributes.SetOrAddAttributeValue(6, float(g_iSkinDefIndex[client][knifeDefIndex]));
                pDynamicAttributes.SetOrAddAttributeValue(7, float(g_iWeaponSkinSeed[client][knifeDefIndex]));
                pDynamicAttributes.SetOrAddAttributeValue(8, g_fWeaponSkinWear[client][knifeDefIndex]);
            }
        }
        else
        {
            // Original code for non-knife weapons
...
public void OnPluginEnd()
{
	...
	CleanupBotGloveConfig();
	CleanupBotKnifeConfig();
}
...
void LoadBotGloveConfig()
{
    ...
    if (FileExists(szPath))
    {
        ...
        delete g_jsonGloveConfig;
        
        g_jsonGloveConfig = JSONObject.FromFile(szPath);
        if (g_jsonGloveConfig != null)
        {
            g_bBotGlovesLoaded = true;
            PrintToServer("[BotGloves] Successfully loaded bot glove configuration.");
        }
        else
        {
            g_bBotGlovesLoaded = false;
            PrintToServer("[BotGloves] Failed to parse JSON file: %s", szPath);
        }
    }
    else
...
void CleanupBotGloveConfig()
{
    delete g_jsonGloveConfig;
    g_bBotGlovesLoaded = false;
}

void LoadBotKnifeConfig()
{
    char szPath[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, szPath, sizeof(szPath), "data/bot_knives.json");
    ...
    if (FileExists(szPath))
    {
        // Clean up any previously loaded knife config
        delete g_jsonKnifeConfig;
        
        g_jsonKnifeConfig = JSONObject.FromFile(szPath);
        if (g_jsonKnifeConfig != null)
        {
            g_bBotKnivesLoaded = true;
            PrintToServer("[BotKnives] Successfully loaded bot knife configuration.");
        }
        else
        {
            g_bBotKnivesLoaded = false;
            PrintToServer("[BotKnives] Failed to parse JSON file: %s", szPath);
        }
    }
    else
...
void CleanupBotKnifeConfig()
{
    delete g_jsonKnifeConfig;
    g_bBotKnivesLoaded = false;
}

public Action Command_ReloadBotConfigs(int client, int args)
{
    LoadBotGloveConfig();
    LoadBotKnifeConfig();
    PrintToServer("[BOTCONFIG] Reloaded glove and knife config files.");
    return Plugin_Handled;
}