...
bool g_bIsRelativePlayback[MAXPLAYERS + 1];
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("BotMimic_PlayRecordFromFileRelative", Native_PlayRecordFromFileRelative);
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
    ...
    // Skip additional teleports for relative playback
    if (!g_bIsRelativePlayback[client] && iFrame.additionalFields & (ADDITIONAL_FIELD_TELEPORTED_ORIGIN|ADDITIONAL_FIELD_TELEPORTED_ANGLES|ADDITIONAL_FIELD_TELEPORTED_VELOCITY))
    {
        AdditionalTeleport iAT;
        ArrayList hAdditionalTeleport;
        char sPath[PLATFORM_MAX_PATH];
    ...
    {
        if (!g_bIsRelativePlayback[client])
        {
            g_bValidTeleportCall[client] = true;
        ...
        {
            // For relative playback, apply velocity and angles without teleport
    ...
    else
    {
        ...
        {
            // For relative playback, apply velocity and angles only
            SDKCall(g_hSetVelocity, client, fActualVelocity);
            TeleportEntity(client, NULL_VECTOR, angles, NULL_VECTOR);
        }
    }
...
public int StopPlayerMimic(Handle plugin, int numParams)
{
    ...
    g_bIsRelativePlayback[client] = false; // Reset relative playback flag
...
public int Native_PlayRecordFromFileRelative(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
    if (client < 1 || client > MaxClients || !IsClientInGame(client))
    {
    ...
    return view_as<int>(PlayRecordRelative(client, sPath));
}

BMError PlayRecordRelative(int client, const char[] path)
{
    if (g_hRecording[client] != null)
    {
        return BM_BadClient;
    ...
    g_iBotActiveWeapon[client] = INVALID_ENT_REFERENCE;
    g_bBotSwitchedWeapon[client] = false;
    g_bIsRelativePlayback[client] = true; // Flag for relative playback
    
    g_iBotMimicNextBookmarkTick[client].BWM_frame = -1;
    g_iBotMimicNextBookmarkTick[client].BWM_index = -1;
    UpdateNextBookmarkTick(client);
    
    // Skip setting initial position and angles
    // Instead, use current position and angles
    GetClientAbsOrigin(client, g_fInitialPosition[client]);
    GetClientEyeAngles(client, g_fInitialAngles[client]);
    ...
    if (result >= Plugin_Handled)
    {
        ...
        g_iBotMimicNextBookmarkTick[client].BWM_frame = -1;
        g_iBotMimicNextBookmarkTick[client].BWM_index = -1;
        g_bIsRelativePlayback[client] = false;
    }
