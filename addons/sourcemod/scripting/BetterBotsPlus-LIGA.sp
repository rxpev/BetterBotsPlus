...
#define FLASH_DODGE_FOV_DOT -1.0
...
void ParseMapStrategies(const char[] szMap)
{
    ...
    else
    {
        ...
        strcopy(szRoot, sizeof(szRoot), "Strategies");
    }
    ...
    KeyValues kv = new KeyValues(szRoot);
...
bool TryAssignStrategy(int strat)
{
    ...
    for (int role = 0; role < g_iStrategyRoleCount[strat]; role++)
    {
        ...
        if (!TryAssignStrategyRole(strat, role, used))
        {
            ClearActiveStrategy(false);
            return false;
        }
    ...
    for (int role = 0; role < g_iStrategyRoleCount[strat]; role++)
    {
        if (role == skippedRole || g_iStrategyRoleUtilityCount[strat][role] > 0)
            continue;
...
bool TryAssignStrategyRole(int strat, int role, bool used[MAXPLAYERS + 1])
{
...
bool DoesAssignedStrategyStillHaveUtility(int strat)
{
    for (int role = 0; role < g_iStrategyRoleCount[strat]; role++)
    {
...
bool DoesClientMeetStrategyUtilityRequirements(int client, int strat, int role)
{
    for (int utility = 0; utility < g_iStrategyRoleUtilityCount[strat][role]; utility++)
    {
        ...
        int heldCount = CountPlayerWeaponsByDefIndex(client, defIndex);
...
void PrintStrategySetupBlocker(int strat)
{
    for (int role = 0; role < g_iStrategyRoleCount[strat]; role++)
    {
        ...
        float fDistance = GetVectorDistance(g_fBotOrigin[client], g_fStrategyRolePos[strat][role]);
        float fSpeed = GetVectorLength(fVelocity);
        bool bOnGround = !!(GetEntityFlags(client) & FL_ONGROUND);
...
int CountPlayerWeaponsByDefIndex(int client, int defIndex)
{
    ...
    char szExpectedClass[32];
    bool bRequireClass = GetStrategyUtilityWeaponClass(defIndex, szExpectedClass, sizeof(szExpectedClass));

    if (offset == -1)
    {
        int weapon = eItems_FindWeaponByDefIndex(client, defIndex);
        if (!IsValidEntity(weapon))
            return 0;

        if (bRequireClass && !IsEntityClassname(weapon, szExpectedClass))
            return 0;
...
bool GetStrategyUtilityWeaponClass(int defIndex, char[] szClass, int iClassLength)
{
    switch (defIndex)
    {
        case 43:
        {
            strcopy(szClass, iClassLength, "weapon_flashbang");
            return true;
        }
        case 44:
        {
            strcopy(szClass, iClassLength, "weapon_hegrenade");
            return true;
        }
        case 45:
        {
            strcopy(szClass, iClassLength, "weapon_smokegrenade");
            return true;
        }
        case 46:
        {
            strcopy(szClass, iClassLength, "weapon_molotov");
            return true;
        }
        case 48:
        {
            strcopy(szClass, iClassLength, "weapon_incgrenade");
            return true;
        }
    }
...
bool IsEntityClassname(int entity, const char[] szExpectedClass)
{
    char szClassname[64];
    GetEntityClassname(entity, szClassname, sizeof(szClassname));
    return strcmp(szClassname, szExpectedClass, false) == 0;
}
...
public void OnFlashbangDetonate(Event eEvent, const char[] szName, bool bDontBroadcast)
{
	...
	fDetonatePos[0] = eEvent.GetFloat("x");
	fDetonatePos[1] = eEvent.GetFloat("y");
	fDetonatePos[2] = eEvent.GetFloat("z");

	float fNow = GetGameTime();
	for (int i = 1; i <= MaxClients; i++)
	{
		if (!g_bFlashDodgeActive[i] || !IsValidClient(i) || !IsFakeClient(i) || !IsPlayerAlive(i))
			continue;
...
stock bool IsFlashDodgeAllowed(int client, bool bIsEnemyVisible)
{
...
{
	float fEyes[3], fAngles[3], fViewDir[3], fToFlash[3], fFlashPos[3];
	GetClientEyePosition(client, fEyes);
	GetClientEyeAngles(client, fAngles);
	GetViewVector(fAngles, fViewDir);
	NormalizeVector(fViewDir, fViewDir);

	int entity = -1;
	float fBestDistance = FLASH_DODGE_MAX_DIST + 1.0;
	bool bFound = false;

	while ((entity = FindEntityByClassname(entity, "flashbang_projectile")) != -1)
	{
		GetEntPropVector(entity, Prop_Data, "m_vecOrigin", fFlashPos);

		float fDist = GetVectorDistance(fEyes, fFlashPos);
		if (fDist > FLASH_DODGE_MAX_DIST)
			continue;

		SubtractVectors(fFlashPos, fEyes, fToFlash);
		NormalizeVector(fToFlash, fToFlash);
		if (GetVectorDotProduct(fViewDir, fToFlash) < FLASH_DODGE_FOV_DOT)
			continue;

		if (fDist < fBestDistance)
		{
			fBestDistance = fDist;
			Array_Copy(fFlashPos, fThreatPos, 3);
			bFound = true;
		}
	}
...
stock void ApplyFlashTurnAway(int client)
{
	...
	GetClientEyePosition(client, fEyes);
	SubtractVectors(g_fFlashThreatPos[client], fEyes, fToFlash);
	NormalizeVector(fToFlash, fToFlash);
...
stock bool HandleFlashDodge(int client, bool bIsEnemyVisible)
{
	...
	float fNow = GetGameTime();
	if (!g_bFlashDodgeActive[client])
	{
		if (fNow < g_fFlashLastDodgeTime[client] + FLASH_DODGE_COOLDOWN)
			return false;
		if (fNow < g_fFlashNextScanTime[client])
			return false;

		g_fFlashNextScanTime[client] = fNow + FLASH_DODGE_SCAN_INTERVAL;
		float fThreatPos[3];
	...
	if (!g_bFlashDodgePopped[client])
	{
		...
		{
			g_bFlashDodgePopped[client] = true;
			g_fFlashDodgeReturnTime[client] = fNow;
		}
		else
		{
			ApplyFlashTurnAway(client);
